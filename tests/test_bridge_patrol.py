...
def _setup_patrol_root(tmp_path: Path, monkeypatch):
    ...
    for path in [
        ...
        bridge / 'audit',
        root / 'scripts',
        bridge / 'incoming' / 'hermes',
        bridge / 'incoming' / 'jarvy',
        bridge / 'incoming' / 'jordan',
        bridge / 'outgoing' / 'hermes',
        bridge / 'outgoing' / 'jarvy',
        bridge / 'outgoing' / 'jordan',
        config.parent,
    ]:
        path.chmod(0o700)
    config.chmod(0o600)
...
def test_patrol_sends_unacknowledged_reminder_once_and_persists_state(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    try:
        ...
        bridge_patrol.main()
        out = capsys.readouterr().out
        assert 'reminder sent for HND-TEST-0001 to jordan' in out
...
def test_patrol_escalates_old_unacknowledged_handoff_without_repeat(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    out = capsys.readouterr().out
    assert 'status: warning' in out
    assert 'escalation: unresolved open handoff for jordan: HND-TEST-0002 age=8.0h' in out
    ...
    bridge_patrol.main()
    out = capsys.readouterr().out
    assert out.count('escalation: unresolved open handoff for jordan: HND-TEST-0002') == 0
...
def test_patrol_escalates_old_acknowledged_handoff_without_resolution(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert 'status: warning' in out
    assert 'escalation: unresolved acknowledged handoff for jordan: HND-TEST-0003 age=7.0h' in out
...
def test_patrol_reminds_on_in_progress_handoff(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    try:
        ...
        assert 'reminder sent for HND-TEST-0004 to jordan status=in_progress age=1.0h count=1' in out
        assert len(notify_server.requests) == 1
...
def test_patrol_warns_on_active_handoff_older_than_alert_threshold_without_actionable_summary(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert 'status: warning' in out
    assert 'active unresolved handoff for jordan: HND-TEST-0005 age=0.8h status=acknowledged summary=pending' in out
...
def test_patrol_skips_active_alert_when_summary_is_actionable(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert 'active unresolved handoff for jordan: HND-TEST-0006' not in out
    assert 'status: ok' in out
